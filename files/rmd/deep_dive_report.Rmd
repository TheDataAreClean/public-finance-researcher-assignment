...
Observation of various Sub-Major and Minor account heads under education to understand trends.
...
FY 2018-19 district spending expenditure transactions.

```{r overview-dist, warning = FALSE}
# Account Heads
district_spend_ed_account_18_19 <- district_spending_exp_final %>%
    filter(demand_desc == "EDUCATION" & year == "2018-19") %>%
    group_by(major_desc, sub_major_desc) %>%
...
# Rename Fields
names(district_spend_ed_account_18_19) <- c("Major", "Sub Major", "Net Payment")

# Print formatted table
formattable(district_spend_ed_account_18_19, 
            list(
...
* Lahaul and Spiti do not have any education spending in 2018-19; even with some schools present in the area.
...
# Education Spending
education_across <- budget_exp_final %>%
    filter(year == "2018-19") %>%
    filter(stringr::str_detect(demand_desc, pattern = "EDUC") | 
           stringr::str_detect(major_desc, pattern = "EDUC")) %>%
    group_by(demand, demand_desc, major, major_desc) %>%
    summarise(sanction = sum(sanction), revised = sum(revised)) %>%
    mutate(percent = (revised - sanction)/sanction) %>%
    arrange(demand, major) %>%
    select(demand, demand_desc, major, major_desc, sanction, revised, percent)
    
# Format % Field
education_across$percent <- formattable::percent(education_across$percent)

# Rename Fields
names(education_across) <- c("Demand", "Demand Desc", "Major", "Major Desc", 
                             "Sanction", "Revised", "Percent")

# Print formatted table
formattable(education_across,
            list(
              Sanction = color_tile("coral", "tomato"),
              Revised = color_tile("lightgreen", "seagreen"),
              Percent = color_tile("paleturquoise", "turquoise")
